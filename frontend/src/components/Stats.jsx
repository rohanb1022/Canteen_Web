...
const Stats = () => {
  ...
  const fetchStats = async () => {
    try {
      if (stats.length === 0) setLoading(true); // Only show loading initially
      setError(null);
      const response = await axiosInstance.get(`/api/v1/statistics?type=${viewType}`);
      ...
      if (response.data.length > 0) {
        setStats(response.data);
        const latest = response.data[response.data.length - 1];
        const previous = response.data.length > 1 ? response.data[response.data.length - 2] : { totalOrders: 0, totalRevenue: 0 };
        setComparison({
          ordersDiff: latest.totalOrders - previous.totalOrders,
          revenueDiff: latest.totalRevenue - previous.totalRevenue,
        });
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      ...
      {activeTab === "orders" && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className="p-4 bg-gray-100 rounded-lg text-center">
              <h3 className="text-lg font-semibold text-gray-700">
                {viewType === "daily" ? "Today vs Yesterday" : "This Month vs Last Month"}
              </h3>
              <p className={`text-xl font-bold ${comparison.ordersDiff >= 0 ? "text-green-600" : "text-red-600"}`}>
                {comparison.ordersDiff >= 0 ? "+" : ""}{comparison.ordersDiff} Orders
              </p>
          ...
          <div className="w-full h-[350px]">
            <Bar data={{
              labels: stats.map(entry => `${entry._id.day}-${entry._id.month}-${entry._id.year}`),
              datasets: [
                { label: "Total Orders", data: stats.map(entry => entry.totalOrders), backgroundColor: "rgba(54, 162, 235, 0.6)" },
                { label: "Completed Orders", data: stats.map(entry => entry.completedOrders), backgroundColor: "rgba(75, 192, 192, 0.6)" }
              ]
      ...
      {activeTab === "revenue" && (
        <>
          ...
          <div className="w-full h-[350px]">
            <Bar data={{
              labels: stats.map(entry => `${entry._id.day}-${entry._id.month}-${entry._id.year}`),
              datasets: [{ label: "Total Revenue", data: stats.map(entry => entry.totalRevenue), backgroundColor: "rgba(255, 99, 132, 0.6)" }]
            }} options={{ maintainAspectRatio: false, plugins: { legend: { position: "top" } } }} />
          </div>
        </>
      )}
